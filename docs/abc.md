...
B+ 树页面没有使用重做日志来维护持久性，或者说除堆页面外的页面都没有，这很棘手，因为堆文件页面会存在 undo 和 redo 来回滚或重做，而这些页面没有。
...
哈希索引文件不需要超级页，HashPage 总是**顺序分配**的。

HashPage 与普通堆文件页类似，以一组 bit 位图开始，随后存储一系列定长的的 HashEntry，HashEntry 存储着 key 和 recordId。

HashPage 中的 HashEntry 数量是固定的，设为 N，并且假定有 M 个页，那么共有 N*M 个槽位，哈希索引将采用线性探测法将 key 映射到这些槽位上。
$$
N = floor(\frac{PageSzie \times 8}{RecordSize \times 8 + 1})
$$
当我们知道一个 key 的 hash 时，将会映射到页号为 $x = \frac{hash \% (N \times M)}{N}$ 的页上，此页上槽位号为 $(hash \% (N \times M)) \% N$ 的槽位上。

### HashPageManager

负责从页面中读取和写入HashPage。 

HashPageManager 同样可以通过 Catalog 获取，对于 HashPageManager 而言，他的 `tableName` 被设置成 `tableName-fieldIndex-indexName`，这样可以保证唯一性。

HashPageManager 还负责单独创建新页，供扩容时使用，创建新页时，必须要原子地维护文件头，为了避免频繁的扩容，每次扩容时都会成倍的创建新页。

### HashPageHolder

使用理由基本与 B+ 树索引一致，HashPageHolder 还负责扩容时上锁，当正在扩容时，不允许其他任何事务操作索引。

哈希索引是基于页锁定的，为了避免死锁，哈希索引一次只会锁定一个页。
...
支持创建索引，一个表必须要有一个主键，主键必须要使用 B+ 树并且必须要在第一列。一个字段可以同时建立哈希和 B+ 树两种索引，但一个字段**不能**具备唯一和非唯一两种特性。
